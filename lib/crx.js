...
exports.init = function(grunt){
  ...
  function expandConfiguration(taskConfig, defaults){
    ...
    ['privateKey', 'src', 'dest'].forEach(function(key){
      taskConfig[key] = resolveHomeDirectory(taskConfig[key]);
    });
    ...
    ['dest'].forEach(function(key){
      taskConfig[key] = grunt.template.process(taskConfig[key], {
        "data": {
    ...
    taskConfig.codebase = taskConfig.baseURL || taskConfig.codebase || null;

    if (taskConfig.codebase && !/.crx$/.test(taskConfig.codebase)){
      taskConfig.codebase = taskConfig.codebase.replace(/\/$/, '');
      taskConfig.codebase += '/' + path.basename(taskConfig.dest);
    }

    // @todo remove that to better separate grunt config and Chrome Extension config
    taskConfig.rootDirectory = taskConfig.src;
    taskConfig.maxBuffer = taskConfig.options.maxBuffer || taskConfig.maxBuffer || undefined;
